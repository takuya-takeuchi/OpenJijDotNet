...
\
DLLEXPORT int32_t graph_Sparse_##__TYPENAME__##_get_J(graph::Sparse<__TYPE__> *sparse, const uint32_t i, const uint32_t j, __TYPE__* value)\
{\
    *value = sparse->J(i, j);\
    return ERR_OK;\
}\
\
DLLEXPORT int32_t graph_Sparse_##__TYPENAME__##_set_J(graph::Sparse<__TYPE__> *sparse, const uint32_t i, const uint32_t j, const __TYPE__ value)\
{\
    sparse->J(i, j) = value;\
    return ERR_OK;\
}\
\
DLLEXPORT int32_t graph_Sparse_##__TYPENAME__##_get_h(graph::Sparse<__TYPE__> *sparse, const uint32_t i, __TYPE__* value)\
{\
    *value = sparse->h(i);\
    return ERR_OK;\
}\
\
DLLEXPORT int32_t graph_Sparse_##__TYPENAME__##_set_h(graph::Sparse<__TYPE__> *sparse, const uint32_t i, const __TYPE__ value)\
{\
    sparse->h(i) = value;\
    return ERR_OK;\
}\
